...
class Layer:
    ...
    def __init__(self,num_inputs, num_outputs, activation, init_method): # inputs = weights number, outputs = neurons number, activation = activation function
        if activation not in ACTIVATIONS:
            raise ValueError(f"Função de ativação '{activation}' não é suportada. Opções: {list(ACTIVATIONS.keys())}")
        
        self.neurons = [Neuron(num_inputs, activation, init_method) for _ in range(num_outputs)] # Create a list of neurons for the layer, each with num_inputs and the specified activation function.
    ...
    def backward(self, inputs, z_list, output_gradients):
        ...
        for e, output_gradient in enumerate(output_gradients):
            input_gradients = self.neurons[e].backward(inputs, z_list[e], output_gradient)
